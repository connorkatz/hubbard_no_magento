<section id="cart_preview">
    <div id="cart_preview_content">
        <h5>Hubbard Free Added to Your Cart</h5>

        <section id="cart_preview_item">
            <div id="cart_preview_item_inner">
                <div id="cart_preview_image">
                    <img src="../images/product_images/hubbard_free_tan_waxhide.jpg">
                </div><!-- end cart preview image -->
                <div id="cart_preview_qty">
                    <label>Qty</label>
                    <div id="cp_qty_select">
                        <span>1 pair</span>
                        <i id="cp_qty_plus" class="fa fa-plus"></i>
                        <i id="cp_qty_minus" class="fa fa-minus"></i>
                    </div><!-- end cp qty select -->
                </div><!-- end cart preview qty -->
                <div id="cart_preview_actions">
                    <a href="#" id="cart_preview_continue">Continue Shopping</a>
                    <a href="/checkout_cart.php" id="cart_preview_checkout">View Cart &amp; Checkout</a>
                </div><!-- end cart preview actions -->
            </div><!-- end cart preview item inner -->
        </section><!-- end cart preveiw item -->

        <div id="cart_preview_crossell">
            <h6>Also Recommended for you</h6>
            <hr class="decorative_break_2">
            <ul>
                <li>
                    <a href="/catalog_product_detail.php">
                        <img src="../images/product_images/off_white/boot_up_black_suede.jpg">
                        <h3><span>Boot-Up</span> Black Suede</h3>
                    </a>
                </li>
                <li>
                    <a href="/catalog_product_detail.php">
                        <img src="../images/product_images/off_white/hubbard_free_almost_black.jpg">
                        <h3><span>Hubbard Free </span> Almost Black</h3>
                    </a>
                </li>
                <li>
                    <a href="/catalog_product_detail.php">
                        <img src="../images/product_images/off_white/hubbard_free_cordovan.jpg">
                        <h3><span>Hubbard Free </span> Cordovan</h3>
                    </a>
                </li>
                <li>
                    <a href="/catalog_product_detail.php">
                        <img src="../images/product_images/off_white/hubbard_free_sand_suede.jpg">
                        <h3><span>Hubbard Free </span> Sand Suede</h3>
                    </a>
                </li>
            </ul>
        </div><!-- end cart preview crossell -->
    </div><!-- end cart preview content -->

</section><!-- end cart preview -->